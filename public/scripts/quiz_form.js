(($) => {
  ...
  const addAnswer = function ($questionForm) {
    ...
    const prevAnswerId =
      $questionForm.find(".answer").last().find("label").attr("for") || null;
  ...
  const addQuestion = function () {
    const questionNum =
      Number($("#quiz_form").find(".question").last().attr("id")) + 1 || 1;
  ...
  const createQuizObj = function (data) {
    ...
    for (const input of data) {
      ...
      if (isNaN(name.charAt(0))) {
        if (!value) {
          return validationError(
            `#${name}`,
            `Please include a ${name.split("_")[1]} for your quiz.`
          );
        }
        if (value.length > 255) {
          return validationError(
            `#${name}`,
            `Quiz ${name.split("_")[1]} must be 255 characters or less.`
          );
        }
        quiz[name] = value;
        continue;
      ...
      if (!value) {
        return validationError(
          `#${name}`,
          `Don't leave any blank questions or answers.`
        );
      }
  ...
  const renderConfirmation = function (data, quiz) {
    let $confPage = $(`<article>
        ...
        <div class="copy_buttons">
          <button class="quizlink_button c_b"><span>Copy Quiz Link</span>&nbsp;<input class="copy_input" value=http://localhost:8080/quiz/${data.url}></button>
          <button class="resultslink_button c_b"><span>Copy Results Link </span> &nbsp;<input class="copy_input" value=http://localhost:8080/quiz/results/${data.resultsUrl}></button>
        </div>
      </article>`);
    ...
    $header
      .children(".conf_private")
      .text(quiz.quiz_private === "TRUE" ? "private" : "public");
